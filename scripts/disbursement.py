...
class Disbursement:
    ...
    def needs_yvyfi(self):
        return self.yvyfi_before / self.yvyfi_to_disperse < EXPECTED_TOKEN_BUFFER


    def needs_yfi(self):
        return self.yfi_before / self.yfi_allocated < EXPECTED_TOKEN_BUFFER
    ...
    def market_buy(self):
        ...
        if self.needs_yfi() and self.needs_yvyfi():
            usdc_to_swap += EXPECTED_TOKEN_BUFFER * usdc_to_swap * self.yvyfi_ratio
    ...
    def transfer_yfi_from_treasury(self):
        ...
        if self.needs_yfi() and self.needs_yvyfi():
            yfi_to_transfer += self.yfi_allocated * EXPECTED_TOKEN_BUFFER * self.yvyfi_ratio
    ...
    def deposit_yfi(self):
        ...
        if self.needs_yvyfi():
            self.yfi_to_deposit += self.yfi_to_deposit * EXPECTED_TOKEN_BUFFER * self.yvyfi_ratio
    ...
    def transfer_yvyfi_from_treasury(self):
        ...
        self.yvyfi_to_transfer = self.yvyfi_to_disperse
        if self.needs_yvyfi():
            self.yvyfi_to_transfer += self.yvyfi_to_transfer * EXPECTED_TOKEN_BUFFER
    ...
    def prep_reward(self):
        ...
        elif self.funding_method == FundingMethod.SEND_YFI:
            assert self.contracts.yfi.balanceOf(self.contracts.safe.account)>= self.yfi_allocated
            token = self.contracts.yfi

        if self.funding_method == FundingMethod.SEND_YFI:
            amount_to_disperse = self.yfi_allocated
        else:
            amount_to_disperse = self.yvyfi_to_disperse

        self.check_buffer(token, amount_to_disperse)
    
    def check_buffer(self, token, amount_to_disperse):
        # Make sure we have a buffer of yvyfi to avoid some errors
        percentage_token_buffer = (
            token.balanceOf(self.contracts.safe.account) - amount_to_disperse
        ) / amount_to_disperse

        assert (
            percentage_token_buffer >= EXPECTED_TOKEN_BUFFER
            ), f"This TX could fail if {token.name()}'s pricePerShare changes before execution.\nThe yvyfi buffer is only {percentage_token_buffer}%\n"
    ...
    def get_amounts(self, coordinape_group_epoch):
        ...
        to_disperse = self.yvyfi_to_disperse
        if self.funding_method == FundingMethod.SEND_YFI:
            to_disperse = Wei(self.yfi_allocated)
        ...
        dust = to_disperse - sum(self.amounts)
        ...
        assert sum(self.amounts) == to_disperse

        if (self.funding_method != FundingMethod.SEND_YFI):
            assert float(self.yfi_allocated) == approx(
                to_disperse * (self.contracts.yvyfi.totalAssets() / self.contracts.yvyfi.totalSupply()),
