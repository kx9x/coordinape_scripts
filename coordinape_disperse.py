...
def disperse(group, epoch, deposit_yfi):
    assert(group in DEFAULT_USD_REWARD_DICT), f"{group.name} does not have a default usd reward entry"
    assert(epoch in DEFAULT_USD_REWARD_DICT[group]), f"{group.name}'s epoch #{epoch} does not have a default usd reward entry"
    ...
    contributors_this_epoch = [contributor for contributor in contributors_from_epoch(group, epoch) if int(contributor['received']) > 0]
    ...
    amounts = [Wei(yvyfi_to_disperse * (Fraction(contributor['received']) / Fraction(total_votes))) for contributor in contributors_this_epoch]
    ...
    assert yfi_allocated == (yvyfi_to_disperse * yvyfi.pricePerShare()) / yfi_decimal_multiplicand
    ...
    l =  [[contributor['name'], contributor['address'][:6], contributor['received'], amount / yfi_decimal_multiplicand, amount / yfi_decimal_multiplicand * yfi_in_usd * pricePerShare] for contributor, amount in zip(contributors_this_epoch, amounts)]
    table = tabulate(l, headers=['Name', 'Address', 'Received Votes', 'Amount yvYFI', 'Amount USD'], tablefmt='orgtbl')
    print(f"{group.name} epoch #{epoch}\nDistributing ${reward_in_usd}\nYFI price ${yfi_in_usd}\nyvYFI price per share {pricePerShare}\n")
    print(table)
